    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try {
                ...
                const acceptedEvents = response.data.filter(event => event.status === 'Accepted');
    ...
    return (
        <div className="relative flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-black via-gray-800 to-black">
            ...
            <div className="relative z-10 max-w-4xl mx-auto">
                    ...
                    {events.map((event, index) => (
                        <div key={index} className="bg-gray-800 p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
                            ...
                            <button
                                ...
                                onClick={() => alert(`You have expressed interest in: ${event.name}`)}
                            >
                                Register
                            </button>
                        </div>
                ...
                {events.length === 0 && (
                    <p className="text-gray-400 mt-4">No accepted events available at this time.</p>
                )}
            </div>
        </div>
    );
