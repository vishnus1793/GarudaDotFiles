        ...
        <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {events.map((event, index) => (
            <div key={index} className="bg-gray-800 p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
              ...
              {/* Button to show registration count */}
              <button
                className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700"
                onClick={() => fetchRegistrationCount(event._id)}
              >
                Show Registered Users
              </button>
              {/* Display the registration count */}
              {registrationCount[event._id] !== undefined && (
                <p className="text-gray-400 mt-2">
                  Registered Users: {registrationCount[event._id]}
                </p>
              )}
            </div>
          ))}
