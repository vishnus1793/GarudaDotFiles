...
const Dashboard = () => {
  const trainDetails = [
    ...
    // Add additional train details as needed
  ];

  const downloadChart = () => {
    const chartElement = document.getElementById("chart"); // Ensure you give your chart a unique ID

    html2canvas(chartElement).then((canvas) => {
      canvas.toBlob((blob) => {
        if (blob) {
          saveAs(blob, "chart.png"); // Use FileSaver to download the image
        }
      });
    });
  };
